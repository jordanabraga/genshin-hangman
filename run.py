...
'''
The section below define variables using the import random to be able
to radomly assign a type of vision to the player.
'''
...
'''
This section defines functions with lists that are referenced in the game.
...
def random_characters():
    genshin_characters = [
        ...
        'Ayaka', 'Ayato', 'Kirara', 'Sara', 'Kuki', 'RaidenShogun', 'Kokomi',
        'Heizou', 'Thoma', 'Yae', 'Yoimiya', 'Charlotte', 'Freminet', 'Furina',
        'Lynette', 'Lyney', 'Neuvillette', 'Wriothesley', 'Navia', 'Clorinde',
        'Sigewinne', 'Arlecchino', 'Baizhu', 'Dainsleif', 'Yaoyao', 'Mika',
        'Yelan', 'YunJin'
    ]
...
The function provides feedback to say if the input is right,
wrong or not valid. It also had a hint system.
    ...
    while not guessed and tries > 0:
        ...
        if len(guess) == 1 and guess.isalpha():
            if guess in guessed_letters:
                ...
                indices = [
                    i for i, letter in enumerate(genshin_characters)
                    if letter == guess
                ]
...
>->->->-> Maybe try the letter {random.choice(genshin_characters)}.'''
        ...
        print("\n˚₊‧꒰ა  ʚ Type 'paimon' to receive a hint. ɞ  ໒꒱ ‧₊˚")
        print("\n")

    if guessed:
        sprint(simple_colors.yellow(
            f'''You got it, {name.capitalize()}!
{genshin_characters.title()} is here to help you!
Together you form the best adventuring due.
No Fatui agent will ever steal your {random_element.capitalize()} vision.\n'''
            ))
    else:
        sprint(simple_colors.yellow(
            f'''Oh no, the Fatui agent stole your vision!
...
print(simple_colors.green(
    "--A Genshin Impact adventure as a Hangman game--\n"
    ))

name = input("\nHello, Traveler! What is your name?\n")

sprint(f"\n{name.capitalize()}, the gods have blessed you with...\n")

sprint(f"the {random_element.capitalize()} vision.\n")

sprint('Oh no! Now a Fatui agent is trying to steal it!\n')

sprint('Can you guess who is coming to help you?\n')

sprint(simple_colors.green(
    '''Instructions: This is a Hangman style game.
